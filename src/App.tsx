...
const computeDiscountFactor = (years: string[], tIdx: number, dataOverride: StaticDataType): number => {
  ...
  for (let k = tIdx - 4; k < tIdx; k++) {
    const yr = years[k];
    pureRevaluedAvg *= dataOverride[yr].i as number;
  }
  let sum = 1;
...
const CAREPensionCalculator: React.FC = () => {
  ...
  const [endYear, setEndYear] = useState<number>(2556);
  ...
  // Handle end year changes with validation
  const handleEndYearChange = (value: string): void => {
